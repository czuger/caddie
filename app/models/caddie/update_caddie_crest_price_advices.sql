...
INSERT INTO caddie_price_advices( eve_item_id, region_id, trade_hub_id, created_at, updated_at )
  ...
  AND NOT EXISTS (
      SELECT NULL FROM caddie_price_advices
      WHERE eve_item_id = eve_items.id
      AND trade_hub_id = trade_hubs.id );

/* Update all datas */
UPDATE caddie_price_advices cpa
...
UPDATE caddie_price_advices cpa
SET adjusted_batch_size = bp.nb_runs*bp.prod_qtt, prod_qtt = bp.prod_qtt
FROM eve_items ei, blueprints bp
WHERE cpa.eve_item_id = ei.id
AND ei.id = bp.eve_item_id;

UPDATE caddie_price_advices cpa SET single_unit_cost = (cost*(1+0.11))/prod_qtt;

UPDATE caddie_price_advices cpa
SET daily_monthly_pcent = min_price / avg_price,
  margin_isk_daily = min_price - single_unit_cost,
  margin_pcent_daily = min_price / (single_unit_cost),
  margin_isk_monthly = avg_price - single_unit_cost,
  margin_pcent_monthly = avg_price / (single_unit_cost)
